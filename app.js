...
const panel = $("panel");
function applyPanelOpen(isOpen){
  localStorage.setItem("panelOpen", isOpen ? "1" : "0");
  if(window.matchMedia("(max-width: 900px)").matches){
    panel?.classList.toggle("open", !!isOpen);
  }else{
    document.body.classList.toggle("panelCollapsed", !isOpen);
  }
  if(map) setTimeout(()=>map.invalidateSize(true), 120);
}
$("sheetHandle")?.addEventListener("click", ()=>applyPanelOpen(!(panel?.classList.contains("open"))));
$("panelToggle")?.addEventListener("click", ()=>{
  const mobile = window.matchMedia("(max-width: 900px)").matches;
  const open = mobile ? !!panel?.classList.contains("open") : !document.body.classList.contains("panelCollapsed");
  applyPanelOpen(!open);
});
window.addEventListener("resize", ()=>{
  const open = (localStorage.getItem("panelOpen") ?? "1") !== "0";
  applyPanelOpen(open);
});
setTimeout(()=>applyPanelOpen((localStorage.getItem("panelOpen") ?? "1") !== "0"), 0);
$("fullscreenBtn")?.addEventListener("click", async ()=>{
  const target = document.querySelector(".mapWrap") || document.documentElement;
  try{
...
function computeScaleAndPercentiles(arr01){
  ...
  if(!vals.length){ state.scaleMin=0; state.scaleMax=1; state.scaleFlat=false; return; }
  const pick = (q)=> vals[Math.min(vals.length-1, Math.max(0, Math.floor(q*(vals.length-1))))];
  let lo = pick(0.02);
  let hi = pick(0.98);
  if(!Number.isFinite(lo) || !Number.isFinite(hi)){
    state.scaleMin = 0; state.scaleMax = 1; state.scaleFlat = false; return;
  }
  if(Math.abs(hi - lo) <= 1e-12){
    const center = lo;
    const eps = Math.max(1e-6, Math.abs(center) * 0.02, 0.01);
    lo = center - eps;
    hi = center + eps;
    state.scaleFlat = true;
  }else{
    state.scaleFlat = false;
  }
  state.scaleMin = lo;
  state.scaleMax = hi;
}
...
function renderOverlay(arr01, conf01){
  ...
  for(let yImg=0;yImg<H;yImg++){
    ...
    for(let x=0;x<W;x++){
      ...
      const minAlphaFloor = (state.map === "conf") ? 0 : 96;
      if(ok && a < minAlphaFloor){
        a = minAlphaFloor;
  ...
  ensurePanes();
  const overlayOpacity = Number.isFinite(state.overlayOpacity) ? state.overlayOpacity : 0.82;
  if(!imageOverlay){
