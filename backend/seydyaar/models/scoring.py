...
def _clean_prob_field(a: np.ndarray, smooth_radius: int = 0, stripe_strength: float = 0.0) -> np.ndarray:
...
def front_score(temp_front: np.ndarray, chl_front: np.ndarray, ssh_front: np.ndarray,
    ...
    s = _clean_prob_field(s, smooth_radius=0, stripe_strength=0.0)
...
def habitat_scoring(inputs: HabitatInputs, priors: Dict, weights: Dict) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    s_temp = _clean_prob_field(score_temp_c(inputs.sst_c, priors["sst_opt_c"], priors["sst_sigma_c"]), smooth_radius=0, stripe_strength=0.0)
    s_chl = _clean_prob_field(score_chl_mg_m3(inputs.chl_mg_m3, priors["chl_opt_mg_m3"], priors["chl_sigma_log10"]), smooth_radius=0, stripe_strength=0.0)
    s_cur = _clean_prob_field(score_current_m_s(inputs.current_m_s, priors["current_opt_m_s"], priors["current_sigma_m_s"]), smooth_radius=0, stripe_strength=0.0)
    s_waves = _clean_prob_field(score_waves_hs(inputs.waves_hs_m, priors.get("waves_hs_soft_max_m", 1.5)), smooth_radius=0, stripe_strength=0.0)
    ...
    if w.get("front", 0.0) > 0.0:
        ...
        s_front = _clean_prob_field(s_front, smooth_radius=0, stripe_strength=0.0)
    ...
    s_eke = _clean_prob_field(robust_normalize(inputs.eke, lo_q=10.0, hi_q=90.0, min_span=5e-4), smooth_radius=0, stripe_strength=0.0) if (inputs.eke is not None and w.get("eke", 0.0) > 0.0) else zero
    ...
    s_npp_anom = _clean_prob_field(np.asarray(inputs.npp_anom, dtype=np.float32), smooth_radius=0, stripe_strength=0.0) if (inputs.npp_anom is not None and w.get("npp_anom", 0.0) > 0.0) else zero
    s_thermocline = _clean_prob_field(np.asarray(inputs.thermocline_proxy, dtype=np.float32), smooth_radius=0, stripe_strength=0.0) if (inputs.thermocline_proxy is not None and w.get("thermocline", 0.0) > 0.0) else zero
    ...
    phab = _clean_prob_field(phab, smooth_radius=0, stripe_strength=0.0)
