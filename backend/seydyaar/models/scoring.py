...
def habitat_scoring(inputs: HabitatInputs, priors: Dict, weights: Dict) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    s_chl_anom = _clean_prob_field(np.asarray(inputs.chl_anom, dtype=np.float32), smooth_radius=1, stripe_strength=0.20) if (inputs.chl_anom is not None and w.get("chl_anom", 0.0) > 0.0) else zero
    s_npp_anom = _clean_prob_field(np.asarray(inputs.npp_anom, dtype=np.float32), smooth_radius=1, stripe_strength=0.20) if (inputs.npp_anom is not None and w.get("npp_anom", 0.0) > 0.0) else zero
    s_thermocline = _clean_prob_field(np.asarray(inputs.thermocline_proxy, dtype=np.float32), smooth_radius=1, stripe_strength=0.10) if (inputs.thermocline_proxy is not None and w.get("thermocline", 0.0) > 0.0) else zero
